...
- [x] 新增 `CaptureAudioRecordingSession` foundation：core 内部已有 SCK system audio、AVCapture mic、writer/mixer/diagnostics、`states` / `levels` streams 的 native session object；sidecar adapter 已接入，app 的 ScreenCaptureKit adapter 也已接入；app CoreAudio fallback 仍保留在 app 层。
...
- [x] ScreenCaptureKit path 下，`AudioRecorder` 降级为 `@MainActor` host adapter：创建 core session、接 sample tap、保持 UI/Cloud/Live Caption 状态在 app 层。
- [x] 通过 core `sampleBufferTap` 复用现有 meter / waveform / Live Caption sample buffer 入口；CoreAudio fallback path 继续使用原 output objects。
...
- [x] 新增 core native `CaptureAudioRecordingSession` foundation，并接入 app 的 ScreenCaptureKit path。
- [x] 把 app 的 ScreenCaptureKit + mic + writer/mixer 流程迁进 core；CoreAudio process tap fallback 仍保留旧 path。
