...
public enum CaptureAudioMixer {
    ...
    public static func mix(sources: [URL], to destination: URL) async throws {
        let readable = try sources.map { url in
            do {
                return try AVAudioFile(forReading: url)
            } catch {
                throw CaptureAudioError.sourceUnreadable(
    ...
    public static func outputHeadroom(forSourceCount count: Int) -> Float {
        count > 1 ? 0.5 : 1.0
    }
}
