...
// Live recording telemetry — aligns with the planned RecordingCore
// `RecordingTelemetry` (+ artifact fields on stop). Runtime assembles this from
// helper events (recording.state / audio.level / recording.artifact) and passes
// it down; the screen only renders.
...
export function RecordingHeroScreen({
  ...
  return (
    <Box flexDirection="column" paddingX={1} height={size.rows}>
      ...
      <Box>
        <Text dimColor>q stop & save · p {paused ? "resume" : "pause"}</Text>
      </Box>
    </Box>
  );
}
